<script module>
  ...
  const maxVal = Math.max(...pncvSubData.map((d) => d.valor));
  const nMax = niceMax(maxVal);
  const innerW = W - M.left - M.right;
  const n = pncvSubData.length;
  ...
  const maxLabelChars = Math.max(...pncvSubData.map((d) => d.label.length));
  ...
  const { Story } = defineMeta({
    ...
    parameters: {
      docs: {
        description: {
          component: `
**Subcategorias da Política Nacional de Cultura Viva (PNCV)**

Distribuição dos recursos dentro da categoria PNCV, mostrando valores absolutos estimados e participação percentual no total geral.
          `,
        },
      },
    },
